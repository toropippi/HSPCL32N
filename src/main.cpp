...
const double PluginVersion = 3.0;
int* aaaa;
...
cl_uint dev_num = 0;//全プラットフォームのデバイスの合計数
...
static int cmdfunc(int cmd)
{
	...
	case 0x50://HCLinit
	{
		...
		//落ちないおまじない
		if (COMMANDQUEUE_PER_DEVICE > 1) 
		{
			aaaa = new int[311 * 1024 * 1024 + dev_num];
			aaaa[284432] = 125345 + dev_num;
			aaaa[2432] = 12354 + dev_num;
		}
		else 
		{
			aaaa = new int[1 * 1024 * 1024 + dev_num];
			aaaa[242] = 125345 + dev_num;
			aaaa[232] = 6234 + dev_num;
		}
		...
		cl_queue_properties prp[3];
		prp[0] = CL_QUEUE_PROPERTIES;// CL_QUEUE_PROFILING_ENABLE;// (cl_queue_properties)cmd_properties;
		prp[1] = CL_QUEUE_PROFILING_ENABLE;
		prp[2] = 0;

		for (int k = 0; k < (int)dev_num; k++)
		{//コンテキストとコマンド級ーを作る
			...
			for (int i = 0; i < COMMANDQUEUE_PER_DEVICE; i++)
			{
				command_queue[k * COMMANDQUEUE_PER_DEVICE + i] =
					clCreateCommandQueueWithProperties(context[k], device_id[k], prp, &errcode_ret);
				//https://www.khronos.org/registry/cl/sdk/2.0/docs/man/xhtml/clCreateCommandQueueWithProperties.html
