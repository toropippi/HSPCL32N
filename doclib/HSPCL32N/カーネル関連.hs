...
２つ以上のデバイス上で同じカーネルを実行したいとき、それぞれのデバイスidをclSetDeviceでセットしなおしてclCreateProgramを実行して下さい。
...
コンパイルされたOpenCLカーネルプログラムは、そのデバイス上でしか使えません。
２つ以上のデバイス上で同じカーネルを実行したいとき、それぞれのデバイスidをclSetDeviceでセットしなおしてclCreateProgramWithSourceを実行して下さい。
...
コンパイルされたOpenCLカーネルプログラムは、そのデバイス上でしか使えません。
２つ以上のデバイス上で同じカーネルを実行したいとき、それぞれのデバイスidをclSetDeviceでセットしなおしてclCreateProgramWithBinaryを実行して下さい。
...
clDoKernelで計算する前にこれでカーネルの引数を予めセットしておかなければいけません。
...
というものなら
clSetKernel p1,0,cl_mem_id_A	//(←clCreateBufferで作成したcl_mem_id)	;配列
clSetKernel p1,1,5	//引数2
と2回に渡り指定します。
...
clSetKernel p1,0,cl_mem_id_dpA
clSetKernel p1,1,cl_mem_id_dpB
clSetKernel p1,2,0,64 (←p3には0を指定、p4は64byteつまりdouble型*8個の共有メモリを作れという意味、OpenCLカーネルソース内ではblock[0]～block[7]が同じワークグループ内で共有して使える)
clSetKernel p1,3,10
...
clSetkrns p1,cl_mem_id_dpA,cl_mem_id_dpB
...
clCreateProgram,clCreateKernel,clSetKernel,clCreateBufferをせずカーネルを実行して結果を得ます。
例えば以下のカーネルコードで
...
この命令はプラグイン内部でclCreateProgram,clCreateKernel,clSetKernel,clCreateBuffer,clWriteBuffer,clReadBuffer,clReleaseKernel,clReleaseProgram,clReleaseBufferを使用しています。
この命令で確保したVRAM等は、この命令が終わるまでに必ず破棄されます。
clDoKernelと違い、タスクが完了するまで次の命令にうつりません。

clCallやclCall2では第一引数の入力文字列がプラグイン内部でハッシュ化され保存されており、全く同じ文字列の場合、前回のビルドを自動で使いまわすことができるようになっています。
つまり毎回カーネルソースをコンパイルしているわけではないので高速です。

しかしclCallでは他の部分の命令のオーバーヘッドも大きいので、速度が求められる場合には向きません。

OpenCL全体のより詳細な解説はclDoKernelを参照下さい。
...
clCreateProgram,clCreateKernel,clSetKernelをせずカーネルを実行して結果を得ます。
clCall2では、今までclCallでHSP配列変数を指定していたところにCL_mem_idを指定します。
...
この命令はプラグイン内部で内部でclCreateProgram,clCreateKernel,clSetKernelを使用しています。
clCallと違いタスクが完了する前に次の命令にうつります(ブロッキングモードoff)。

clCallやclCall2では第一引数の入力文字列がプラグイン内部でハッシュ化され保存されており、全く同じ文字列の場合、前回のビルドを自動で使いまわすことができるようになっています。
つまり毎回カーネルソースをコンパイルしているわけではないので高速です。
ただclDokrn1,2,3命令のように引数指定と分離できてるわけではないので、細かいことを言えば引数指定の分オーバーヘッドはどうしてもあります。
...
clCallでは自動的に処理していた「HSPで用意した配列変数をVRAM等に移す処理」「スレッドの次元設定」を
自ら設定しなければなりません。
しかし設定できる部分が少ないclCallと比較して処理の高速化や自由度の高いことが可能になります。


HSPユーザーとしてclDoKernel（OpneCLプラグイン）を利用する時、理解の上で躓きやすい点をリストアップしました。
そのためにOpenCLを利用する上での独特な処理を説明します。

	1.OpenCL用の命令ソースを別途用意する必要がある。（「OpenCL C」という。VecAdd.clなど）
	そしてそのファイルをHSP上で固有の命令（clCreateProgram）で読み込み。
	これによりProgramオブジェクトが生成される。これは64bitプラグインではint64型、32bitプラグインではint型の数値で表される。

	2.そのProgramオブジェクトから「カーネル」と言われる命令単位を抽出する。HSP上で固有の命令（clCreateKernel）で作成する。
	これによりKernelオブジェクトが生成される。これは64bitプラグインではint64型、32bitプラグインではint型の数値で表される。

	3.GPU上のメモリの配列を確保する。HSP上で固有の命令clCreateBufferにて作成する。
	これによりCL_memオブジェクトが生成される。これは64bitプラグインではint64型、32bitプラグインではint型の数値で表される。
	
	4.CPU→GPUにデータ転送する。
	clWriteBufferを使いHSP配列をCL_mem idのさすGPUメモリへ転送する。
	
	5.カーネル関数へ引数をセットする。
	Kernelオブジェクトと、CL_memオブジェクトのidを使って、clSetKernel命令で引数セットをおこなう。

	6.カーネルを固有の命令（clDoKernel）で実行。
...
この図全体がNDRange
(clDoKernel kernel_id,2,[4,6],[2,3] で実行したときを想定)
...
html{
<img src="./doclib/HSPCL32N/thumbs/fermioverview.jpg">
}html
...
html{
<img src="./doclib/HSPCL32N/thumbs/fermistreamingmultiprocessor.png">
}html
...
%inst
動作はwork_dimが1の場合のclDoKernelと同じです。
...
clDoKrn1ではglobal_work_sizeがlocal_work_sizeで割り切れなければいけませんでした。
...
clSetDeviceで指定しているデバイスのすべてのコマンドキューに入れられた全てのOpenCLコマンドを発行します。
...
clSetDeviceで指定しているデバイスのうち、使えるコマンドキューは0～3まであります。
デフォルトでは0です。
...
%inst
clSetCommandQueueでセットした番号を返します。
;--------
%index
clReleaseKernel
カーネル破棄
...
■型については
global変数の場合、clDoXiはint型に決定されます。
...
例えばclDoXi命令で、サイズが256*4=1024byteのcl memをp2に指定した場合
...
■大文字変数について(2文字以上)
1文字の大文字はglobal変数と解釈されますが、型については例えばclDoXdなら全てdoubleと決め打ちされてしまいます。
...
2回目以降の実行はclDokrn1,2,3と同じくらい、気にならない程度のオーバーヘッドになるはずです。
例えば1秒に10000回HCLDoX系命令を実行するなら別ですが・・・その場合clDokrn1,2,3系命令のほうが明らかにオーバーヘッドという観点では高速になるでしょう。(もちろんGPU上のカーネルコードの実行速度は変わらない)
...
int p1:[C]cl_mem_id			[in]
int p2:[A]cl_mem_id			[in]
int p3:[B]cl_mem_id			[in]
...
A,BにはclBLAS_Set2DShape
であらかじめcl memに行、列のサイズを設定しておく必要があります。

命令として実行することもできますが、関数として実行することもできます。
その際は
C=clBLAS_sgemm(A,B,0,0,0)
のように使います。
この場合、Cには新たにclCreateBufferで確保されたmem idが返されます。
...
clBLAS_sgemm命令やclBLAS_dgemm命令を使う際にあらかじめ行、列を正しく設定しておく必要があります。
この設定した行と列の値はcl memと一緒にプラグイン内部に記録され、clBLAS_Get2DShapeで取り出すことができます。
...
%prm
int p1,int p2,int p3,int p4,int p5,int p6
int p1:[y]cl_mem_id			[in]
int p2:[A]cl_mem_id			[in]
int p3:[x]cl_mem_id			[in]
...
AにはclBLAS_Set2DShape
であらかじめcl memに行、列のサイズを設定しておく必要があります。

x,yはclBLAS_Set2DShapeでサイズ指定されていても無視されます。
...
y=clBLAS_sgemm(A,x)
のように使います。
この場合、yには新たにclCreateBufferで確保されたmem idが返されます。
